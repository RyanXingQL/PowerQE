...
class SeparableConv2d(nn.Module):
    ...
    def __init__(self, nf_in, nf_out):
        ...
        self.separable_conv = nn.Sequential(
            ...
            nn.Conv2d(in_channels=nf_in, out_channels=nf_out, kernel_size=1, groups=1),  # then point-wise
        )
...
class GaussianSmoothing(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        return self.conv(x, weight=self.weight, groups=self.groups, padding=self.padding)
...
class IQAM:
    ...
    def cal_tchebichef_moments(self, x):
        x = x.clone()
        x /= torch.sqrt(self.patch_sz * self.patch_sz * (x.reshape((-1, )).pow(2).mean()))
        x -= x.reshape((-1, )).mean()
                ...
                if sum_patch == 0:
                    ...
                    if ssm > self.thr_smooth:
                        ...
                        patch_blurred = torch.squeeze(
                            self.gaussian_filter(patch.clone().view(1, 1, self.patch_sz, self.patch_sz)))
                        moments_patch_blurred = self.cal_tchebichef_moments(patch_blurred)
                        similarity_matrix = torch.div(
                            (torch.mul(moments_patch, moments_patch_blurred) * 2.0 + self.bigc),
                            (moments_patch.pow(2)) + moments_patch_blurred.pow(2) + self.bigc,
                        )
                        score_blurred_textured += 1 - torch.mean(similarity_matrix.reshape((-1)))
                        ...
                        strength_vertical = (
                            torch.sum(torch.abs(moments_patch[self.patch_sz - 1, :])) / sum_moments -
                            torch.abs(moments_patch[0, 0]) + self.bigc)
                        strength_horizontal = (
                            torch.sum(torch.abs(moments_patch[:, self.patch_sz - 1])) / sum_moments -
                            torch.abs(moments_patch[0, 0]) + self.bigc)
                        ...
                        score_ = torch.log(1 - (
                            (strength_vertical + strength_horizontal) / 2)) / torch.log(1 - self.thr_jnd)
...
class Down(nn.Module):
    ...
    def __init__(self, nf_in, nf_out, method, if_separable, if_eca):
        ...
        if method not in supported_methods:
            raise NotImplementedError(f'Downsampling method should be in "{supported_methods}";'
                                      f' received "{method}".')

        if if_separable and if_eca:
            layers = nn.ModuleList([ECA(k_size=3), SeparableConv2d(nf_in=nf_in, nf_out=nf_in)])
            ...
            layers = nn.ModuleList([nn.Conv2d(
                in_channels=nf_in,
                out_channels=nf_in,
                kernel_size=3,
                padding=3 // 2,
...
class Up(nn.Module):
    ...
    def __init__(self, nf_in_s, nf_in, nf_out, method, if_separable, if_eca):
        ...
        if method not in supported_methods:
            raise NotImplementedError(f'Upsampling method should be in "{supported_methods}";'
                                      f' received "{method}".')
...
class RBQE(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(repeat_times):
            if if_separable and if_eca:
                self.out_layers.append(nn.Sequential(ECA(k_size=3), SeparableConv2d(nf_in=nf_base, nf_out=nf_io)))
            elif if_separable and (not if_eca):
        ...
        for idx_unet in range(self.nlevel):  # per U-Net
            ...
            for idx_up in range(idx_unet + 1):
                ...
                for idx_, feat_level in enumerate(feat_level_unet[-(idx_up + 1):]):
                    dense_inp_list.append(feat_level[idx_])  # append features from previous U-Nets at the same level
