...
class QENet(nn.Module):
    ...
    def __init__(self, in_nc=64, nf=48, nb=6, out_nc=3, base_ks=3):
        ...
        for _ in range(nb):
            body += [nn.ReLU(inplace=True), nn.Conv2d(nf, nf, base_ks, padding=padding)]
        body += [nn.ReLU(inplace=True), nn.Conv2d(nf, out_nc, base_ks, padding=padding)]
...
class STDFNet(BaseNet):
    ...
    def __init__(
        ...
        self.stdf = STDF(
            in_nc=io_channels * (radius * 2 + 1),
            out_nc=nf_stdf_out,
            nf=nf_stdf,
            nb=nb_stdf,
            deform_ks=deform_ks,
        )
        self.qe_net = QENet(in_nc=nf_stdf_out, nf=nf_qe, nb=nb_qe, out_nc=io_channels)
